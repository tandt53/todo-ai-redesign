...
`specs/_shared/platform/web.md ## Test Harness` delegates "e2e (Playwright) is
...
3. **`vitest.config.ts`** (root, new) — `test.exclude` covers
   `qa/**/automation/e2e/**` and `qa/**/automation/harness/**`. Root cause:
...
Full utterance→scenario mapping: `qa/_shared/fixtures/web/assistant-web-fixtures.json`.
This mirrors what **qa-api-agent's own execute-phase run found independently**
(`qa/assistant/runs/2026-08-16-api-execute.md`): the authored api suite had
