...
- `specs/assistant/api-contracts.md` §9 fixes the value as
  `alternative: "danh sách và bộ lọc trên màn hình"` ("the on-screen list and
  its filters").
- `specs/assistant/data-model.md:95` pins the same literal on `TurnOutcome`.
...
1. **`src/assistant/web/__tests__/model.test.ts:430,434`** — builds its own
   ...
   Owner: web-agent.
2. **`qa/assistant/F-001/web/TC-002, TC-024, TC-030`** — quote the old English
...
Note that `specs/assistant/F-001-voice-assistant-view.md:102` (AC-15) describes
