...
`distance_from_bottom = 121`, one affordance reading `1 new message`, and the reply to the user's own turn is below the fold. Reproduced 3 of 3 isolation runs. The scroll starts, animates, and stops 121 logical units short of the bottom — the height of the row that appended while it was moving. Filed as `qa/_shared/bugs/BUG-006-own-send-lands-short-of-the-bottom.md`, layer `web`.
