...
## Expected behaviour
- **NMA-NEW** carries the count label published by `design/_shared/components.md` §NewMessageAffordance (`1 new message`), an accessible name of `{label}, scroll to newest`, and a border that is **not** the `question` accent.
- **NMA-WAITING** carries `Waiting for your answer — {question}` where `{question}` is the pending question's own head, verbatim (`Delete 3 tasks?`), an accessible name of `{label} Scroll to newest`, and the published `question` accent from `design/_shared/tokens.json`.
...
| expected copy | parsed at run time from `design/_shared/components.md` §NewMessageAffordance |
| expected accent | parsed at run time from `design/_shared/tokens.json` (`color.*.question`) |
