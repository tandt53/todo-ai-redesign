...
## Preconditions
- Account `qamob-tc001@qa.example.com`, open session, seeded tasks per the canonical utterance→intent table (`docs/qa/assistant/F-001/api/utterance-intent-fixtures.json` — read, never copied).
...
| utterances | canonical rows from `docs/qa/assistant/F-001/api/utterance-intent-fixtures.json` (edit + create) |
| namespace | `qamob-` |
