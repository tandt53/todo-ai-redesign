...
stays simulator-free per `docs/specs/_shared/platform/mobile.md ## Test Harness`.
...
`docs/qa/_shared/bugs/BUG-002-cold-open-accepts-input-before-session-read.md`
...
`docs/qa/assistant/F-001/api/utterance-intent-fixtures.json`, never copied.
...
| `docs/qa/assistant/F-003/mobile/*.md` (40 TCs) | 35 flipped to `active` + `automated`; TC-039 rewritten for `resource-id` with the reasoning; TC-030 and TC-021 carry execution results; **TC-040 added** (foreground-as-reconnect) |
| `docs/qa/assistant/F-003/mobile/index.md` | execute-phase status block, resolved drift items, BUG-002 entry, restated debt, corrected tally |
| `docs/qa/_shared/bugs/BUG-002-cold-open-accepts-input-before-session-read.md` | new |
| `docs/qa/assistant/runs/2026-08-17-mobile-execute.md` | this record |
...
4. **architect-agent** — add the announcement seam (`model/announce.ts`,
   `RecordingAnnouncer`) to `docs/specs/_shared/platform/mobile.md`'s port list. It is
...
5. **design-agent** — fold the per-combination Vietnamese permission copy into
   `docs/design/_shared/components.md`. It currently lives only in the implementation.
