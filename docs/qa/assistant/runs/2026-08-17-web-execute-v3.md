...
| `docs/qa/_shared/fixtures/web/assistant-web-fixtures.json` | `hoàn tác` row removed; new QA_EXTRA row documented; provenance note added |
| `docs/qa/assistant/F-001/web/*.md` | 22 test-case documents |
| `docs/qa/assistant/F-001/web/index.md` | copy-language section, run-record links, header provenance |

### The copy block

`EN` keeps the shape `VN` had — one home for every copy expectation, functions where a string carries a live count or title. Values trace to `docs/design/_shared/components.md` and the mockup `docs/design/assistant/screens/voice-assistant-view.html`, not to the implementation, except for three strings noted under "Catalogue gaps".
...
`docs/qa/assistant/F-001/api/utterance-intent-fixtures.json` — the QA half of the canonical utterance→intent table, the exact artifact L-004 was written about — still declares three rows the executable table no longer has:
...
Three strings asserted by the automation have no published home in `docs/design/_shared/components.md` or the mockups, and were taken from the implementation under this task's rule that the implementation wins unless it contradicts the catalogue:
