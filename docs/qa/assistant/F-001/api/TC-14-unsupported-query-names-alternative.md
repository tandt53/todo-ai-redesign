...
Re-synced twice, both times because the **contract** moved first and the server followed it — never to make a red test green against unchanged behaviour. 2026-08-16 (T-016b): the literal became Vietnamese in the T-015g Gate-3 localization pass. 2026-08-17 (T-070a): it returned to English, `the on-screen list and its filters`, per ADR-008 (English is the product language) — `docs/specs/assistant/api-contracts.md` §9 rule 9 and `docs/specs/assistant/data-model.md` TurnOutcome are the source this TC copies, and both already carried the English string before this edit. The assertion stays an exact-equality check on a verbatim literal (never imported from `src/`), so re-wording the refusal copy still fails this test.
