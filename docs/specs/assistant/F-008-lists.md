...
- ~~**OQ-3**~~ — **Closed (T-286): widen to `string`.** The four static members become named constants. A personal list's collection id is its uuid. A branded type or a tagged prefix (`list:${uuid}`) adds ceremony without safety: the runtime value is already a uuid, and prefixing it means every comparison and preference key must add/strip the prefix. Widening to `string` makes every `switch` on `Collection` require a `default` branch, which is exactly the forcing function that ensures new call sites handle list ids. See `data-model.md § Collection type`.
