...
**ID**: F-003
**Slug**: mobile-surface
**Status**: `draft` (revision 1. **Amended 2026-08-22 (T-229)**: parity table updated for F-001 revision 8's four new ACs (AC-33 through AC-36). **AC-13 is new** — the mobile-specific delete accessibility path via VoiceOver rotor / TalkBack custom action, carrying F-001 AC-33's mobile clause. 12 become 13; no AC was renumbered or deleted.)
...
designed_in:       [docs/design/assistant/screens/voice-assistant-view-ios.html, docs/design/assistant/screens/voice-assistant-view-android.html]
api_endpoints:     []   # no new endpoints — mobile is a second client against F-001's eight
tested_by:
  api:    []
  web:    []
  mobile: [docs/qa/assistant/F-003/mobile/, tests/assistant/mobile/F-003-mobile-surface.spec.ts, src/assistant/mobile/__tests__/touch-keyboard-back.test.ts, docs/qa/assistant/runs/2026-08-17-mobile-execute.md]
known_bugs: []
```
...
F-001's ACs are the behaviour contract. **Revision 8 adds AC-33, AC-34, AC-35 and AC-36; all four are enumerated below.** *(AC-30, AC-31 and AC-32 from revisions 4–7 are still owed an enumeration — see `index.md`.)* Their disposition on mobile, in full — this table is the enumeration AC-1 binds:

- **Hold identically; QA cites the F-001 id, no mobile fork (22):** AC-1, AC-2, AC-4, AC-6, AC-7, AC-8, AC-9, AC-10, AC-11, AC-12, AC-13, AC-14, AC-15, AC-16, AC-17, AC-18, AC-20, AC-22, AC-23, AC-24, AC-29, AC-34. Nine of these (AC-6, AC-10, AC-12, AC-15, AC-16, AC-23, plus the api halves of AC-1, AC-7, AC-9) are server-side and cost the mobile client no behaviour at all. AC-34 (gesture partitioning) holds identically: tap title = inline edit, tap row = detail, checkbox = complete.
- **Hold, with a mobile clause added here (7):** F-001 AC-3 → AC-7 (audio interruption); F-001 AC-5 → AC-9 (one-gesture undo by touch); F-001 AC-19 → AC-12 (native screen-reader announcement); F-001 AC-21 → AC-2 / AC-3 (the permission split); F-001 AC-25 → AC-4 (offline recognition); F-001 AC-28 → AC-8 (foreground resume trigger); **F-001 AC-33 → AC-13 (delete accessibility path — VoiceOver rotor and TalkBack custom action)**. The F-001 behaviour continues to hold in full — the clause adds, it never narrows.
...
- **Web-only, does not apply to mobile (2):** F-001 AC-35 (Lists drawer close at `wide` breakpoint — web viewport resize has no mobile analogue), F-001 AC-36 (no add-task above fold at `split`+ — the mobile layout is always below `split`).
...
### Delete accessibility path (rev 8 — F-001 AC-33's mobile clause)
- [ ] **AC-13** (ios, android) — **Delete on a task row is a left-swipe reveal, and the swipe is not reachable by VoiceOver or TalkBack.** F-001 AC-33 requires delete to be reachable without gesture; this AC carries the mobile-specific paths. Delete is available through: **(1)** the Delete control in the task detail (F-005), reached by tapping the row; **(2)** a **VoiceOver rotor custom action** labelled "Delete task" on iOS (`accessibilityCustomActions`), and a **TalkBack custom action menu** entry labelled "Delete task" on Android (`AccessibilityNodeInfoCompat.addAction`). The custom action performs the same `DELETE /tasks/{id}` and carries the same undo (F-005 AC-42, AC-43). The action's accessible label matches the swipe-revealed control's label (WCAG 2.5.3). **Verified against a real screen reader on a device**, not inferred from the accessibility tree — the same standard AC-12 holds. **This AC joins the device-pass debt**: the custom action's discoverability is a rendered behaviour with no headless observable.

## Verification status

**Read this before reading a ticked box as "works on a phone."** The checkboxes above record what the executed test tier proved, and that tier is deliberately device-free: it runs under Node with vitest, with everything native mocked at the port boundary (`docs/specs/_shared/platform/mobile.md ## Test Harness`). No suite in this repository has ever run on a simulator, an emulator, or a device. A green run is therefore evidence about the model and the ports — never about the OS.

- **Ticked — AC-1 … AC-8.** Verified by assertions that actually executed, against the same conversation reducer the web client uses. See `docs/qa/assistant/F-003/mobile/index.md` for the run record and per-TC status.
- **Unticked — AC-9, AC-10, AC-11, AC-12, AC-13.** *Specified and modelled, not verified.* These five are not merely untested-so-far: **they have no headless observable at all.** A touch target's hit area, a keyboard occluding a message, a system-back gesture routing to the background path, what VoiceOver or TalkBack actually speaks, and whether a VoiceOver/TalkBack custom action is discoverable are all rendered or OS behaviour. Asserting a prop instead would test source text rather than the observable (`docs/specs/_shared/LEARNINGS.md` L-002). Their node-testable halves do run; that is not the same claim, and their boxes stay empty until a device pass ticks them.
- **Status: conditionally accepted.** F-003 is accepted for sign-off *pending a device pass*. The condition is part of the acceptance, not a note attached to it — it is not discharged by any further node-tier work, however green.
- **What the device pass owes** is enumerated once, in `docs/qa/assistant/F-003/mobile/index.md ## Device-lab and manual debt` (8 items with their ACs and TCs), and summarised in this spec's `## Test strategy (mobile)`. It is not duplicated here — one list, one home, so it cannot drift into two versions that disagree.
- **A ticked box is not a device pass either.** Several ticked ACs carry device-lab residue on top of their verified node half — real permission dialogs and the Settings deep link (AC-2, AC-3), a real interrupting call (AC-7), a real OS memory-pressure kill including the storage flush before process death (AC-5, AC-6), and on-device recognition offline in the interface language (AC-4). For those, ticked means *the node half is proven*, never *the AC is fully verified*. The kill case is the sharpest: a storage write that is never awaited passes every headless test and still loses the user's words on a real device.
...
- **Unit tier runs under Node with vitest — no simulator, no emulator, no Metro** (`docs/specs/_shared/platform/mobile.md ## Test Harness`: `npx vitest run src/assistant/mobile`). React Native is not imported by `model/` or `ports/`, which is what keeps the tier device-free; anything native is mocked at the port boundary, never inside a test.
