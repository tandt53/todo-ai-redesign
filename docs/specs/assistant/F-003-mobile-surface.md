...
- **Unticked — AC-9, AC-10, AC-11, AC-12.** *Specified and modelled, not verified.* These four are not merely untested-so-far: **they have no headless observable at all.** A touch target's hit area, a keyboard occluding a message, a system-back gesture routing to the background path, and what VoiceOver or TalkBack actually speaks are all rendered or OS behaviour. Asserting a prop instead would test source text rather than the observable (`docs/specs/_shared/LEARNINGS.md` L-002). Their node-testable halves do run; that is not the same claim, and their boxes stay empty until a device pass ticks them.
