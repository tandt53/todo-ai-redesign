...
**ID**: F-009
**Slug**: list-actions
**Status**: `draft` (**Revision 1, 2026-08-22 (T-234)**: AC-6 amended in place — screen-reader reorder is deferred by owner decision, with the deferral explicitly bounded to reorder (delete is not covered; F-001 AC-33 stands). **AC-15 is new**: Tab navigation to every task row and every row control, unconditional on sort mode. The lists-vs-tasks reorder inconsistency is recorded as accepted in `## Out of Scope`. No AC was renumbered or deleted; 14 become 15.) (**Revision 2, 2026-08-22 (T-243)**: AC-15 re-tagged `(web, mobile)` and reworded — the requirement is now sequential navigation with per-platform realisation named for web and mobile, not Tab specifically. The mobile half anchors the existing `TaskList.tsx` accessibility structure where each row control is independently reachable by VoiceOver and TalkBack. AC-15 now names its link to AC-6's screen-reader reorder deferral in one clause. No AC was renumbered or deleted; count unchanged at 15.) (**Revision 3, 2026-08-22 (T-245)**: Owner settled Gate 1.5 M1 — completed tasks are selectable. AC-9 adds an explicit clause; AC-11 states that bulk complete applies uniformly regardless of current status. The design's exclusion of done rows from selection was considered and rejected, recorded in AC-9. Drag-reorder of done tasks remains open (OQ-2). No AC was renumbered or deleted; count unchanged at 15.) (**Revision 4, 2026-08-22 (T-250)**: AC-9 exit-control wording updated — "Done button" replaced with ✕ icon button per `components.md` vocabulary table (T-249). No AC renumbered or deleted; no substance changed; count unchanged at 15.)

## Links

- primary_module: assistant
- depends_on: [F-001, F-005, F-006, F-008]
- designed_in: [docs/design/_shared/components.md, docs/design/assistant/screens/app-shell.html, docs/design/assistant/screens/app-shell-ios.html, docs/design/assistant/screens/app-shell-android.html]
- implemented_in: [src/assistant/web/components/TasksSurface.tsx, src/assistant/web/styles.css]
- api_endpoints: []
- tested_by: { api: [], web: [src/assistant/web/__tests__/app.test.tsx], mobile: [] }
...
- [ ] **AC-9** (web, mobile) — **Select** in the overflow menu enters multi-select mode: selection checkboxes appear on each row, a toolbar at the bottom shows bulk actions, a selected-count is displayed. Completed rows are included — a done task shows a selection checkbox and is selectable; its selected-count contribution is the same as any other row. Exit: **✕** icon button (accessible name "Exit selection") or deselect all. **Excluding completed tasks from selection was considered and rejected** (owner decision 2026-08-22, T-245): bulk-complete on a done task is a no-op in effect, which made exclusion defensible, but the cost is worse — bulk delete (AC-12) and bulk move (AC-13) cannot reach completed tasks, so clearing forty finished tasks becomes forty single deletions.
...
- [ ] **AC-11** (web, mobile) — **Bulk complete**: marks all selected tasks as done. Applied uniformly regardless of current status — an already-complete task receives the same operation with no special case, no error, and no skip. The toolbar does not disable Complete based on the completion state of the selection.
...
- **OQ-2 (AC-6)** — **Drag-reorder of completed tasks.** The design excludes done rows from drag-reorder (no drag handle) for the same reason it excluded them from selection. The owner settled the selection half (completed tasks are selectable, T-245) but did not address drag. Two candidates: **(A)** done rows get a drag handle like any other row, consistent with the selection decision; **(B)** done rows remain non-draggable, because reorder is cosmetic (display order within a filing cell, not task data or membership) and a done task's position matters less than its reachability for bulk operations. AC-6's screen-reader deferral reasoning ("reorder is cosmetic") supports B; the selection decision's reasoning ("excluding done rows blocks bulk delete/move") does not extend to drag because drag does not gate any destructive or membership-changing operation.
