...
3. Read the files BRIEFING.md lists under "Read these files first", typically:
   ...
   - 1–2 existing API test files for pattern matching (these are existing automation,
     NOT source code — under qa/{module}/automation/api/)
...
- `qa/{module}/automation/e2e/` or `qa/{module}/automation/mobile/`
- Any file under `src/`
...
5. Draft the automation file(s) at qa/{module}/automation/api/F-{id}-{slug}.{spec-ext}.
...
```
1. Run the automation suite from qa/{module}/automation/api/F-{id}-*.
