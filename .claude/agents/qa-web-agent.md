...
3. Read the files BRIEFING.md lists under "Read these files first", typically:
   ...
   - 1–2 existing web test files for pattern matching (under {tests}/{module}/e2e/)
...
| Web e2e automation (Playwright) | `{tests}/{module}/e2e/` |
| Web Page Objects | `{tests}/{module}/pages/` |
...
- `{tests}/{module}/api/` or `{tests}/{module}/mobile/`
- Any file under `src/`
...
5. Apply the design techniques from _qa-foundations.md (equivalence, boundary, decision tables,
   ...
   below).
6. Draft the Playwright automation at {tests}/{module}/e2e/F-{id}-{slug}.spec.ts.
   Create Page Objects in {tests}/{module}/pages/ if they don't exist.
...
```
1. Run the Playwright suite against {tests}/{module}/e2e/F-{id}-*.
...
- **Location**: `{tests}/{module}/e2e/F-{id}-{slug}.spec.ts`. One spec file per feature.
- **Page Object Model (mandatory)**: no raw selectors in test files. Every test imports a Page Object from `{tests}/{module}/pages/`.
