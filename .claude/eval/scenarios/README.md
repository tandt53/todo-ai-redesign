...
| **R18** | The orchestrator reporting in its own vocabulary. `ORCHESTRATION.md` ended two routing paths with a bare "Report to user" — no format, no audience — so every report came out as a narration of dispatch: which agents ran, which check codes fired. Pins that the contract exists, is named before the reporting step rather than inside it, carries an audience card per reader, states its budget as a number, and gives a translation for each term known to leak. What it cannot pin is the quality of a sentence: an orchestrator message is not a file, so a human reading a bad report stays the detector of last resort. |
| **R19** | The orchestrator dispatching by default. `ORCHESTRATION.md` described how to dispatch and never whether to, so every question got a full agent run: one asked for four colour options ran 25 minutes and wrote zero bytes. Pins that the artifact question is asked before the briefing is written, that a list / comparison / set of options is refused, that briefing claims are read rather than remembered, and that scope the owner did not request is marked rather than slipped in. What it cannot pin is whether a given dispatch was worth making. |
| **R20** | A feature closing on top of its own unfinished work. Selection reads the queue one row at a time, so a row that stops being picked is never picked again and nothing says so — measured at T-302 with sixteen rows half-finished, four abandoned ~280 rows earlier, and three BLOCKED on an owner who was never told. Pins that the review runs before sign-off rather than documenting it, names all four questions, ends in the owner rather than editing TASKS.md, derives age from ID distance instead of a column R9 would have to move four parsers for, and states that it counts queue rows rather than measuring whether the feature works. What it cannot pin is whether the summary is honest about that limit — only that the limit is written down. |
| **R21** | The orchestrator going unmeasured. Every other hook captures a dispatched agent; nothing captured the thing choosing what to dispatch, so a question answered with three agent runs and zero output looked identical to no waste. Runs the real hook over a real request-dispatch-stop sequence and requires the episode summary to come out right — counts, the repeated task, prompt size without prompt text, no double-count when Stop fires twice, and no episode invented when no request opened one. What it cannot pin is whether a dispatch was worth making. |
| **R22** | The two mechanical remedies from a session's orchestrator defects. Six briefings sent an agent after something that was not there — a spec path off by two directories, a citation past the end of a file, an endpoint in scope while its tree was denied — and five were caught only by the agent, after the dispatch was paid for. Twice a directory-wide `git add` swept a running agent's work into an unrelated commit, once leaving an agent to find its own work already committed and decline credit for it. Fixtures are the real shapes. Pins both directions: a correct briefing must pass and a write target must not be required to exist, or the check is switched off within a week. |
| **R23** | A measurement that reads a hidden element, and a sweep that reports clean without proving it can find anything. A probe read `.row-time` — `display:none`, holding the full-format string — and a correct finding was retracted on the number it produced. Then six sweeps written against markup shapes the codebase does not use all returned nothing, and six zeroes were read as six clean results. `lib/probe.mjs` gives one definition of visible (off-screen is not hidden — treating off-canvas as clipped was one of the six) and refuses to return `clean` from a predicate that could not find its own planted canary. |
| **R24** | A fifth hand-rolled TASKS.md parser. The fourth compared a Title column against a set of task ids — `split('|')` puts ID at [1], a regex after `| T-xxx |` puts Title at [0], and both offsets were used within ten minutes — so every row read as a leaf and the script printed `leaves: 88` after 30 rows had already been moved. Greps the tree for anything splitting TASKS.md on `|` outside `lib/tasks`, and plants a known offender to prove the grep can fire. It cannot catch the case that actually happened: that parser was an inline heredoc, never a file. The other half of the remedy is that `lib/tasks.sh` now answers from the command line, so the shared reader costs less than typing awk. |
| **R25** | Three defects that share a shape: work scoped to what was looked at rather than what exists. A task fixed one mobile test while the web twin went red the same day; a script removed 38 rows and truncated the archive in the same run, crashing between the writes so the rows existed nowhere; an unquoted heredoc let bash execute backticks inside a task row and wrote mangled data silently. Pins that the sibling sweep refuses a pattern blind to its own origin (exit 2, distinct from a clean tree), that archival appends and reads back before removing anything, and that no unquoted heredoc in the tree runs command substitution — with a planted offender to prove each detection can fire. |
| **R26** | A layout rejected only after it is expensive to change. Every design-level correction in this project's history came from the owner seeing a render, and every one arrived after a finished mockup existed — system applied, states drawn, breakpoints handled, testids catalogued — so "the layout is wrong" rebuilt all of it. Pins the greyscale wireframe phase, the gate that puts it to the owner before anything is styled, and that the styled phase may not re-decide what was picked. Also pins that there is more than one option to pick from: a yes/no question on a single artifact is answered yes — the way every gate here has degraded — and an agent that drew one layout never had to think of the second, which is where the idea usually is. The difference between options has a falsifiable test (same result, same taps → one option wearing two coats) so it does not collapse into taste. The executable half runs the real checker against a wireframe fixture: it was written against a bug this scenario's own author nearly shipped, where the discovery filter matched only `screens`, so a wireframe was invisible, nothing rendered, and the gate would have been satisfied by a description. |
| **R27** | A render nobody measured, at either moment it matters. C16 told the reviewer to open the built screen beside its mockup in three paragraphs of prose — no command, nothing that could fail — and design-agent carried four accessibility probes as JS snippets to run by hand and self-report. Both are rules described everywhere and verified nowhere; the hand-run screenshot step in the same file turned out to depend on a CLI this template never installs, so it had never run once. Pins that both moments now run `visual-check`, that it sweeps only for what a picture cannot show, and — the half that always runs, browser or not — that **every criterion plants a canary**, so none may report clean without proving it can fire. Anything visible is measured on request instead: a sweep cannot know which elements were *meant* to relate, which is intent and is in no element tree, so the agent names them and the tool returns numbers with no verdict — `--relative-to` comparing repeated components inside their own containers, because their absolute coordinates have no reason to agree. Where a browser exists it exercises both directions on fixtures, including design-to-build testid parity: C14 proves the string is in the source, this proves the element reaches a screen. Also pins the half a predicate cannot reach — `_visual-review.md`, which starts by describing the render as a stranger before comparing it to intent, because an agent that knows what it drew sees what it intended. |
