# `specs/_source/` — inherited requirements, verbatim and read-only
...
`ADR-7`, `ADR-11` all appear in `specs/assistant/` — while the documents themselves lived
...
the redesign, **write that in `specs/assistant/` and cite the UC** — the divergence then
...
**3. `ADR-N` here is NOT `ADR-00N` in `specs/_shared/adr/`.** The two projects number ADRs
independently and the ranges overlap: this directory's `01-architecture.md` discusses
ADR-6, ADR-7, ADR-8, ADR-11 and ADR-12, while `specs/_shared/adr/` holds ADR-001 … ADR-008
...
A feature spec in `specs/assistant/` should **cite** the UC it implements
