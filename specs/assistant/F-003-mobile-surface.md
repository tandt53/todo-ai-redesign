...
**Read this before reading a ticked box as "works on a phone."** The checkboxes above record what the executed test tier proved, and that tier is deliberately device-free: it runs under Node with vitest, with everything native mocked at the port boundary (`specs/_shared/platform/mobile.md ## Test Harness`). No suite in this repository has ever run on a simulator, an emulator, or a device. A green run is therefore evidence about the model and the ports — never about the OS.

- **Ticked — AC-1 … AC-8.** Verified by assertions that actually executed, against the same conversation reducer the web client uses. See `qa/assistant/F-003/mobile/index.md` for the run record and per-TC status.
- **Unticked — AC-9, AC-10, AC-11, AC-12.** *Specified and modelled, not verified.* These four are not merely untested-so-far: **they have no headless observable at all.** A touch target's hit area, a keyboard occluding a message, a system-back gesture routing to the background path, and what VoiceOver or TalkBack actually speaks are all rendered or OS behaviour. Asserting a prop instead would test source text rather than the observable (`specs/_shared/LEARNINGS.md` L-002). Their node-testable halves do run; that is not the same claim, and their boxes stay empty until a device pass ticks them.
...
- **What the device pass owes** is enumerated once, in `qa/assistant/F-003/mobile/index.md ## Device-lab and manual debt` (8 items with their ACs and TCs), and summarised in this spec's `## Test strategy (mobile)`. It is not duplicated here — one list, one home, so it cannot drift into two versions that disagree.
