...
Path (a) was chosen then and is what `design/_shared/components.md`,
`specs/assistant/F-002-talk-back.md` AC-23 and `specs/assistant/data-model.md`
...
remains `design/_shared/components.md`, and the L-008 arrangement — consumers
...
- Bad, and **accepted deliberately**: with no i18n layer, **a third language
  ...
  `qa/` files, 14 in `specs/`. This is a tradeoff the owner priced and took; it
...
- **Copy stops having one owning artifact.** The direct-replacement cost stays
  bounded only because `design/_shared/components.md` owns every string and
...
`design/_shared/components.md` is the owning artifact and several tests parse it
...
and the spec edits that accompany it change only `specs/`.
