...
`specs/_source/todo-ai/02-use-cases.md` (UC-01 … UC-54, contiguous, 54 of them)

## Why this file exists, and where it lives

`specs/_source/` was imported today. This repo has cited it since F-001 — `UC-20`,
...
(`specs: specs/`, `shared_dir: _shared`). Three roots were candidates and two are wrong:

- **not `specs/_source/`** — that root is a verbatim, read-only mirror (`specs/_source/README.md`
...
- **not `specs/assistant/`** — the module root holds features. This file spans the whole product,
...
- **`specs/_shared/`** — where cross-cutting artifacts live by MANIFEST convention
...
1. **A UC is evidence of what was wanted, not a standing commitment.** These were written for the
   ...
   tally look tidy.
2. **`specs/_source/` is not edited.** Every reinterpretation lives here or in an F-doc.
3. **`todo-ai ADR-7` is not `ADR-007`.** Two independent ranges that overlap. Inherited ADRs are
   written `todo-ai ADR-N`; this repo's are zero-padded (`specs/_shared/adr/ADR-00N`). T-066
...
- If a UC is reinterpreted for the redesign, the reinterpretation belongs in an **F-doc**, and
  this map records where — never in `specs/_source/`.
